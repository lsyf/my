...
import com.loushuiyifan.common.mapper.ResourceMapper;
import com.loushuiyifan.system.dao.ResourceDAO;
...
public class ResourceService {
    ...
    public List<Resource> getResources(String type) {
        ...
        if ("all".equals(type)) {
            ...
            resources = resourceDAO.getResourceByType(type);
        }
        return resources;
    }
    ...
    public int updateResource(String type, Long resourceId, String name) {
        Resource resource = resourceDAO.getResourceByTypeAndId(type, resourceId);
        resource.setName(name);
    ...
    public int deleteResourceWithKids(String type, long resourceId, String paths) {
        int num = resourceDAO.deleteResourceByTypeAndId(type, resourceId);
        num += resourceDAO.deleteResourceKids(type, paths);
        return num;
    }
    ...
    public int updateResource(String ids, int alls) {
        ...
        return resourceDAO.updateAllsByIds(resourceIds, alls);
    }
    ...
    public boolean isResourceRelatedRole(long resourceId, String type) {
        //首先判断该资源引用数量
        int num = resourceDAO.getRoleByResourceId(resourceId, type);

        //再加上下级资源引用数
        String path = resourceDAO.getResourcePath(resourceId, type);
        path = path + resourceId + "/%";
        num += resourceDAO.getRoleByResourceKids(path, type);
